...
def test_dashboard_allows_authorized_user(client):
    _login_test_user(client, ["iam-dashboard-access", "identity-viewer"])
...
def test_dashboard_denies_unauthorized_user(client):
    _login_test_user(client, ["identity-viewer"])
...
def test_unauthorized_user_receives_custom_403_page(
    ...
    page_text = " ".join(response.get_data(as_text=True).split())
